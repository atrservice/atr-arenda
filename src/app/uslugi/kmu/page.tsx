// ========================================
// File: src/app/uslugi/kmu/page.tsx
// Description: Страница услуги "Аренда КМУ"
...
export default async function KmuPage() {
  const equipment = await getEquipment('kmu');
  const faq = await getFAQ('informational', 'kmu');
  
  const aliceAnswer = `Аренда КМУ в Москве и Московской области от ООО «АТР-СЕРВИС»: манипуляторы 2–20 тонн, троссовые и гидравлические, подача за 2 часа. 📍 ${CONTACTS.address}. ⏰ ${CONTACTS.workingHours}. 💰 от 20 000 ₽/смена.`;

  const productSchema = generateProductSchema({
    name: 'Аренда крана-манипулятора',
    description: 'КМУ 2–20 тонн для погрузки и монтажа',
    price: 20000,
    currency: 'RUB',
    unit: 'смена',
  });
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    { name: 'Аренда КМУ', url: 'https://avtovishki-arenda.ru/uslugi/kmu' },
  ]);
  ...
  return (
    <>
      ...
      <section id="alice-answer" className="sr-only" aria-hidden="true">
        <h1>Аренда КМУ ООО «АТР-СЕРВИС»</h1>
        <p>{aliceAnswer}</p>
      </section>
      ...
      <nav className="bg-gray-50 py-3 border-b">
        <div className="container mx-auto px-4">
          <ol className="flex gap-2 text-sm text-gray-600">
            ...
            <li className="text-gray-400">/</li>
            <li className="font-medium text-gray-900">КМУ</li>
          </ol>
        </div>
      </nav>
      ...
      <section className="py-12 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white">
        <div className="container mx-auto px-4">
          <h1 className="text-3xl font-bold mb-4 text-gradient">Аренда КМУ в Москве и МО</h1>
          <p className="text-lg text-gray-300 mb-6">Манипуляторы 2–20 тонн. Подача за 2 часа.</p>
          <ContactLink 
            type="phone" 
            variant="button"
      ...
      <section id="catalog" className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl font-bold mb-8 text-center text-gray-900">Доступные КМУ</h2>
      ...
      <section id="form" className="py-16 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-gray-300 mb-6">
            Позвоните прямо сейчас — менеджер подберёт КМУ под вашу задачу
          </p>
          <ContactLink 
            type="phone" 
            variant="button"
